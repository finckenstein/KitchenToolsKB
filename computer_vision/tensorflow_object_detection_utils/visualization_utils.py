...
def get_value(div_values):
    # print("get_value: ", div_values, type(div_values))
    for item in div_values.values():
        # print(item)
        return item
    return None
...
def find_boxes_coordinates_for_tool(coordinates, tools):
    list0 = []
    for elem_list in coordinates:
        coordinates_by_score_dic = {round(elem_list[4]): [elem_list[0], elem_list[1], elem_list[2], elem_list[3]]}
        list0.append(coordinates_by_score_dic)

    # print("LIST0: ", list0)
    list1 = []
    for elem in tools:
        tool_score_list = elem.split(": ")
        tool = tool_score_list[0]
        score = int(tool_score_list[1][:-1])
        # print(tool, score)
        list1.append({score: tool})
    # print("LIST1: ", list1)
    #
    # print("\n")

    set_of_tool_boxes = []
    for coordinate_by_score_dic, tool_by_score_dic in zip(list0, list1):
        # print("coordinates: ", coordinate_by_score_dic.keys(),  "& tools dic: ", tool_by_score_dic.keys())
        # print("coordinates type: ", type(coordinate_by_score_dic.keys()), "& tools dic: ", type(tool_by_score_dic.keys()))

        # assert len(coordinate_by_score_dic.keys()) <= 1 or len(tool_by_score_dic.keys()) <= 1, "there are more keys then expected"
        # assert coordinate_by_score_dic.keys() != tool_by_score_dic.keys(), "keys don't match. Something is wrong"
        # assert len(coordinate_by_score_dic.values()) <= 1 or len(tool_by_score_dic.values()) <= 1, "more values than expected"

        coordinates = get_value(coordinate_by_score_dic)
        # print("tool is: ", coordinates)
        tool = get_value(tool_by_score_dic)
        # print("coordinates are: ", tool)
        score = get_score(coordinate_by_score_dic)

        set_of_tool_boxes.append({tool: [coordinates, score]})

    return set_of_tool_boxes


def get_coordinates_for_tool_found(
        image,
        boxes,
        classes,
        scores,
  ...
  """
    ...
    return find_boxes_coordinates_for_tool(coordinates_list, display_array)
